...
const VALID_BITS = [1, 2, 4, 8, 16, 32, 64, 128]
const INVALID_BITS = new Array(555).fill(0).map((_, i) => i).filter((v) => !VALID_BITS.includes(v))
...
describe("NiftyupNFT", function () {
  ...
  describe("Create new NFT", () => {
    ...
    VALID_BITS.map((v) => {
      context(`With ${v} bits`, () => {
        describe('implementation contracts', () => {
          ...
          it('Should start with no implementation', async () => {
            const addr = await factory.implementations(v)
            expect(addr).to.equal(ethers.constants.AddressZero)
          })
    
          it('Should create implementation after first deployment', async () => {
            await factory.create(signers[0].address, v)
            const addr = await factory.implementations(v)
            expect(addr).to.not.equal(ethers.constants.AddressZero)
          })
    
          it('Should use same implementation on second deployment', async () => {
            await factory.create(signers[0].address, v)
            const addr1 = await factory.implementations(v)
            await factory.create(signers[0].address, v)
            const addr2 = await factory.implementations(v)
    ...
    describe('Reject invalid bits values', () => {
      INVALID_BITS.map((v) => {
        it(`Should reject packed with ${v} bits`, async () => {
          const tx = factory.create(signers[0].address, v)
          await expect(tx).to.be.reverted
        })
      })
    })
